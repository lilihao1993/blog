...
public class LoginController {
    ...
    public AjaxResponse<Boolean> register(User user, Model model) throws Exception {
        AjaxResponse<Boolean> response = new AjaxResponse<>();
        //1.判断邮箱是否已注册
        if (userService.isEmailExist(user.getMail())) {
            response.failure(AjaxResponse.ERROR_EMAIL_EXIST, "邮箱已经存在");
            return response;
        }
        //2.判断昵称是否存在
        if (!userService.isUserNameExist(user.getUserName()).isEmpty()) {
            response.failure(AjaxResponse.ERROR_USERNAME_EXIST, "昵称已经存在");
            return response;
        }
        //3.注册用户
        return new AjaxResponse<>(userService.addUser(user));
    }
    ...
    public AjaxResponse<Boolean> login(User user, Model model) throws Exception {
        AjaxResponse<Boolean> response = new AjaxResponse<>();
        //1.根据用户名获取用户
        List<User> userList = userService.isUserNameExist(user.getUserName());
        if (userList.isEmpty()) {
            response.failure(AjaxResponse.ERROR_USER_EXIST, "用户不存在");
            return response;
        }
        //2.输入的密码 + 数据库获取的盐值 进行MD5密码加密
        String passwordMD5 = EncryptUtils.encryptPassword(user.getPassword(), userList.get(0).getEncryptSalt());
        //3.判断密码是否正确
